...
def score_signal(
    ...
    def add(name: str, score: int, rationale: str) -> None:
        contributions.append(
            FactorContribution(name=name, score=score, rationale=rationale)
        )
    ...
    if features.opening_volume_multiple >= config.volume_multiple_threshold:
        move_score = (
            1 if features.first_15m_return > 0 else -1 if features.first_15m_return < 0 else 0
        )
        if move_score:
            add(
                "opening_volume_multiple",
                move_score,
                "opening participation confirms direction",
            )

    if features.gap_hold_or_fade == "holding":
        add(
            "gap_hold_or_fade",
            2 if features.gap_pct > 0 else -2 if features.gap_pct < 0 else 0,
            "gap is holding",
        )
...
def _map_confidence(raw_score: int, contributions: list[FactorContribution]) -> int:
    if not contributions:
        return 0
    same_direction = (
        sum(1 for item in contributions if item.score > 0)
        if raw_score > 0
        else sum(1 for item in contributions if item.score < 0)
    )
